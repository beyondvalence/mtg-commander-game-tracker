-- Fix create_game_with_participants: resolve participant_user_id directly
-- from the player upsert RETURNING clause instead of a separate SELECT.
-- Eliminates N extra index lookups (one per participant) per game creation.
