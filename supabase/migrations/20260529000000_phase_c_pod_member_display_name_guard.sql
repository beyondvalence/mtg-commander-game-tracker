-- Phase C: enforce that every participant in a pod game is an identity-linked
-- pod member (players.linked_user_id IS NOT NULL and that user is in the pod).
-- Arbitrary player names are rejected at the DB level.
...
begin
  ...
  loop
    ...
    -- Phase C guard: reject names that don't belong to an identity-linked pod member.
    -- Checks players.linked_user_id → pod_members.user_id for p_pod_id so the DB
    -- enforces the same membership criteria as fetchAddGamePlayerSuggestions.
    if not exists (
      select 1
      from public.players p
      join public.pod_members pm on pm.user_id = p.linked_user_id and pm.pod_id = p_pod_id
      where p.display_name = trim(v_participant->>'player_name')
        and p.linked_user_id is not null
    ) then
      raise exception 'player "%" is not an identity-linked pod member — they must set their display name on /me first', trim(v_participant->>'player_name');
    end if;

    -- Upsert player and resolve commander owner in one round-trip.
    -- coalesce(linked_user_id, v_user_id): use the player's own auth user if
    -- they've linked their identity, else fall back to the game creator.
    ...
    returning id, coalesce(linked_user_id, v_user_id) into v_player_id, v_participant_user_id;
